using Wuxi.Shengshen.Erp.ApiService.Infrastructure.Validation;
...
{
    /// <summary>验证码盐值请求头名称（前端从响应头读取后回传）。</summary>
        ...
        IOptions<CaptchaOptions> options)
    {
        ...
        // 生成盐值并写入 Redis（仅存 code，salt 在响应头返回，前端回传即可校验）。
    ...
    private static async Task<IResult> AccountLogin(
        [FromBody] Data.Requests.Login.LoginRequest request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var handle = await @lock.TryAcquireAsync(
            ...
            cancellationToken: cancellationToken) ?? throw new BusinessException("登录请求过于频繁，请稍后重试");
        ...
        if (user.IsDelete) throw new BusinessException("用户不存在");
        if (user.IsDisable) throw new BusinessException("用户已封禁，请联系管理员");
        ...
        var token = await tokenService.IssueTokenAsync(user.Id);
